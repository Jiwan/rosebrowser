...
var config = yamlConfig.load(path.normalize(__dirname + '/config.yml'));
...
var bounds: { [index: string]: Array<Array<number>> } = {};
var mapsDir = path.normalize(config.data.local + '/3ddata/maps');
...
fs.writeFileSync(path.normalize(config.data.client_path) + '/js/helpers/mapbounds.js', comment + data);
